...
# 删除 drop
obj = pd.Series(np.arange(5.), index=[i for i in 'abcde'])
new_obj = obj.drop('c')
new_obj
obj.drop(['d', 'c'])
...
# 索引、选取和过滤
obj = pd.Series(np.arange(4.), index=[i for i in 'abcd'])
obj['b']
obj[1]
obj[2:4]
obj[['b', 'a', 'd']]
...
s2 = pd.Series([-2.1, 3.6, -1.5, 4, 3.1], index=[i for i in 'acefg'])
s1 + s2
